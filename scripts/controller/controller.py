...
class Controller:
    ...
    def _update_stage_on_timer(self):
        """Main timer loop - delegates to stage-specific handlers."""
        try:
    ...
    def _select_alarm_state(self, tag):
        """Dispatch alarm states to appropriate handlers."""
        try:
    ...
    def _test_temper(self):
        try:
            self.signals.lab_win_test.emit()
            self.state.last_max_temper = -100.0
    ...
    def _test_lab_cascade(self):
        try:
            ...
            self.state.max_cascade = len(self.model.data_test.speed_list)
